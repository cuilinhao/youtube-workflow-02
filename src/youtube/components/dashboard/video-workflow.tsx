...
import { 
  ...
  PromptEntry
} from '@youtube/lib/types';
import { api } from '@youtube/lib/api';
import defaultWorkflow from '@youtube/data/default-video-workflow.json';
import pLimit from 'p-limit';
import { ensureRemoteImageUrl } from '@youtube/lib/r2/r2Upload';
import { toast } from 'sonner';
import { VideoTaskBoard } from '@youtube/components/dashboard/video-task-board';
import { ShotPromptEditor } from '@youtube/components/workflow/shot-prompt-editor';
import { VideoPromptEditor } from '@youtube/components/workflow/video-prompt-editor';
import {
  VideoTaskForm,
  createEmptyVideoTaskDraft,
  type VideoTaskFormSubmitPayload,
} from '@youtube/components/dashboard/video-task-form';
...
export function VideoWorkflow() {
  ...
  const selectFolderMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch('/api/youtube/system/select-folder', { method: 'POST' });
      if (!response.ok) {
  ...
  const requestReorder = useCallback(async (payloadImages: GeneratedImage[]) => {
    ...
    const response = await fetch('/api/youtube/reorder', {
      method: 'POST',
  ...
  const handleGenerateShots = async () => {
    ...
    try {
      const data = await retryWithBackoff(
        async () => {
          const response = await fetch('/api/youtube/shot-prompts', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  const handleGenerateVideoPrompts = async () => {
    ...
    try {
      ...
      const data = await retryWithBackoff(
        async () => {
          const response = await fetch('/api/youtube/video-prompts', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
              script, 
